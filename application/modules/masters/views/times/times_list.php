	...
	<div class="col-12">
		...
		<div class="card">
			<div class="card-header">
				<h3 class="card-title">
					<span><a href="<?= site_url('masters/times/create-new'); ?>" class="btn btn-xs btn-success"><i class="fa fa-plus"></i> New </a></span>
							...
							<table id="example1" class="table table-bordered table-striped dataTable dtr-inline table-sm text-sm" aria-describedby="example1_info">
								<thead>
									<tr>
										...
										<th class="text-center" width="8%">Member</th>
									...
									<?php if ($times) :  $slNo = 1; ?>
										<?php foreach ($times as $val) : ?>
											<tr class="odd">
												...
												<td><?= $val->prayer_time; ?></td>
												<td class="text-center"><?= $val->allowed_member; ?></td>
												<td class="text-center"><a href="<?= site_url('masters/times/edit/' . $val->id); ?>" class="btn btn-xs btn-success"><i class="fas fa-edit"></i></a></td>
											</tr>
										<?php
										$slNo++;
										 endforeach; ?>
		...
		$('#delete_btn').on('click', function() {
			...
			if (post_arr.length > 0) {
				if (confirm("Do you really want to delete records?")) {
					$.ajax({
						type: "POST",
						url: base_url + "masters/times/delete",
						async: false,
						cache: false,
						data: {
							ids: post_arr
