	...
	<div class="col-12">
		...
		<div class="card">
			<div class="card-header">
				<h3 class="card-title">
					<span><a href="<?= site_url('masters/terms/create-new'); ?>" class="btn btn-xs btn-success"><i class="fa fa-plus"></i> New </a></span>
									...
									<?php if ($terms) :  $slNo = 1; ?>
										<?php foreach ($terms as $val) : ?>
											<tr>
												...
												<td><?= $val->language; ?></td>
												<td><?= $val->title; ?></td>
												<td class="text-center"><a href="<?= site_url('masters/terms/edit/' . $val->id); ?>" class="btn btn-xs btn-success"><i class="fas fa-edit"></i></a></td>
											</tr>
										<?php
										$slNo++;
										 endforeach; ?>
		...
		$('#delete_btn').on('click', function() {
			...
			if (post_arr.length > 0) {
				if (confirm("Do you really want to delete records?")) {
					$.ajax({
						type: "POST",
						url: base_url + "masters/terms/delete",
						async: false,
						cache: false,
						data: {
							ids: post_arr
