        ...
        <script>
        	$(function() {
				...
				$('#group_no').on('change', function(){
					...
					$.ajax({
						type:"post",
						url : base_url+ 'prayerchain/printdata/get_members',
						data : "group_no="+$groupNo+"&centerId="+centerId,
						cache : false,
						async : false,
						success : function(response){
							$('#members').empty();
							...
							var data = JSON.parse(response);
							for (let i = 0; i < data.length; i++) {
								html += '<option value="'+ data[i].id +'">'+ data[i].bro_sis +' '+ data[i].eng_name +'</option>';
							}
							$('#members').append(html);
						}
					});
			...
			function get_member_wise_data(){
				var form_data = $('#quickForm').serializeArray();
				 $.ajax({
					type : "POST",
					url : base_url + 'prayerchain/printdata/get_member_wise_data',
					data : form_data,
					cache : false,
					async : false,
					success : function(response){
