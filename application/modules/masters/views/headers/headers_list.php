	...
	<div class="col-12">
		...
		<div class="card">
			<div class="card-header">
				<h3 class="card-title">
					<span><a href="<?= site_url('masters/headers-data/create-new'); ?>" class="btn btn-xs btn-success"><i class="fa fa-plus"></i> New </a></span>
					...
					<div class="row">
						<div class="col-sm-12">
							<table id="example1" class="table table-bordered table-striped dataTable dtr-inline table-sm text-sm" aria-describedby="example1_info">
								...
								<tbody id="show_data">
									<?php if ($headers) :  $slNo = 1; ?>
										<?php foreach ($headers as $val) : ?>
											<tr>
												<td class="text-center" tabindex="0"><?= $slNo; ?></td>
												<td><input type="checkbox" name="ids[]" id="ids_" value="<?= $val->id; ?>"></td>
												<td><?= $val->language; ?></td>
												<td><?= $val->title; ?></td>
												<td class="text-center"><a href="<?= site_url('masters/headers-data/edit/' . $val->id); ?>" class="btn btn-xs btn-success"><i class="fas fa-edit"></i></a></td>
											</tr>
										<?php
										$slNo++;
										 endforeach; ?>
		...
		$('#delete_btn').on('click', function() {
			...
			if (post_arr.length > 0) {
				if (confirm("Do you really want to delete records?")) {
					$.ajax({
						type: "POST",
						url: base_url + "masters/headers-data/delete",
						async: false,
						cache: false,
						data: {
							ids: post_arr
